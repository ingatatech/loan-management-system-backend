...
// Ensure only one operational fund per organization
@Entity("operational_funds")
@Unique(["organizationId"]) // This ensures only one record per organization
export class OperationalFunds {
  ...
  // Get current balance (alias for amountCommitted)
  getCurrentBalance(): number {
    return this.toDecimal(this.amountCommitted);
  }
