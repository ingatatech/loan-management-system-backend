...
import { LoanStatus, InterestMethod, RepaymentFrequency } from "../entities/Loan";
...
router.post(
  ...
  [
    ...
    body("nationalId").trim().isLength({ min: 16, max: 16 }).withMessage("National ID must be exactly 16 characters"),
    body("gender").isIn(Object.values(Gender)).withMessage(`Gender must be one of: ${Object.values(Gender).join(", ")}`),
    ...
    body("address").custom((value) => {
      ...
      if (typeof value === 'string') {
        try { addressObj = JSON.parse(value); } 
        catch (e) { throw new Error('Address must be valid JSON'); }
      }
      if (typeof addressObj !== 'object') throw new Error('Address must be an object');
      return true;
    }),

    // ✅ REMOVED: Validation for 6 fields
    // - annualInterestRate
    // - disbursementDate
    // - agreedMaturityDate
    // - repaymentFrequency
    ...
    // Loan basic validation (only what's required for pending status)
    body("purposeOfLoan").trim().isLength({ min: 5, max: 500 }),
    body("branchName").trim().notEmpty(),
    body("loanOfficer").trim().notEmpty(),
    body("disbursedAmount").isFloat({ min: 1000 }).withMessage("Loan amount must be at least 1,000 RWF"),
...
router.post(
  ...
  [
    ...
    // ✅ Validation for the 6 approval fields
    ...
    body("disbursementDate")
      .isISO8601()
      .withMessage("Disbursement date must be a valid ISO date")
      .custom((value) => {
    ...
    body("agreedMaturityDate")
      .isISO8601()
      .withMessage("Agreed maturity date must be a valid ISO date")
      .custom((value, { req }) => {
