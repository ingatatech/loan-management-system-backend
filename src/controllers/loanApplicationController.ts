...
import { LoanApplicationService, ServiceResponse } from "../services/loanApplicationService";
import { BorrowerProfile } from "../entities/BorrowerProfile";
import { Loan, LoanApprovalData, LoanStatus } from "../entities/Loan";
...
import { ExtendedGuarantorData } from "../entities/Guarantor";
...
class LoanApplicationController {
 ...
 private loanWorkflowService: LoanWorkflowService;
  ...
  this.loanWorkflowService = new LoanWorkflowService(
    dbConnection.getRepository(LoanWorkflow),
    dbConnection.getRepository(Loan),
    dbConnection.getRepository(User),
    dbConnection.getRepository(LoanReview)
  );
  }
...
getLoanReviews = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
 ...
 createLoanApplication = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      console.log('=== CREATE LOAN APPLICATION (PENDING) DEBUG START ===');
      ...
      const borrowerData = {
        ...
        notes: req.body.borrowerNotes,
      };

      // ✅ REMOVED: 6 fields are NOT in loanData anymore
      const loanData = {
      ...
      const collateralData = {
        ...
        notes: req.body.collateralNotes,
      };
      ...
      console.log('=== CREATE LOAN APPLICATION DEBUG END ===');
      ...
      console.error("Create loan application controller error:", error);
