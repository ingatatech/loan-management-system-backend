...
class LoanApplicationController {
  ...
  createLoanApplication = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const collateralData = {
        ...
        location: req.body.collateralLocation || undefined,
        ownerInfo: req.body.collateralOwnerInfo
          ? (typeof req.body.collateralOwnerInfo === 'string'
            ? JSON.parse(req.body.collateralOwnerInfo)
            : req.body.collateralOwnerInfo)
          : undefined,
        marketValue: req.body.marketValue ? parseFloat(req.body.marketValue) : undefined,
        insuranceValue: req.body.insuranceValue ? parseFloat(req.body.insuranceValue) : undefined,
        forcedSaleValue: req.body.forcedSaleValue ? parseFloat(req.body.forcedSaleValue) : undefined,
      };
      ...
      console.log('📁 Files received from frontend:', Object.keys(files || {}));
      ...
      const borrowerFiles: BorrowerFiles = {
        ...
        loanRelevantDocuments: files?.loanRelevantDocuments || [], // ✅ FIXED: Added missing field
      };
      ...
      const institutionFiles: InstitutionFiles = borrowerType === BorrowerType.INSTITUTION ? {
        ...
        institutionRelevantDocuments: files?.institutionRelevantDocuments || [], // ✅ FIXED: Added missing field
      } : {
      ...
      if (result.success) {
        // ── Audit: loan application created ──
        SecurityService.createAuditLog({
          action: AuditAction.LOAN_CREATED,
          resource: AuditResource.LOAN,
          resourceId: String(result.data?.loan?.id ?? ''),
          userId: req.user?.id,
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan application created — amount: ${loanData.disbursedAmount}, borrower type: ${borrowerType}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId: result.data?.loan?.loanId,
            borrowerType,
        ...
        SecurityService.createAuditLog({
          ...
          description: `Loan application creation failed: ${result.message}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: { reason: result.message, borrowerType, disbursedAmount: loanData.disbursedAmount },
        }).catch(err => console.error('Audit log error:', err));
  ...
  rejectAndCloseLoan = async (
    ...
    try {
      ...
      if (result.success) {
        // ── Audit: loan rejected and closed ──
        SecurityService.createAuditLog({
          ...
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan rejected and closed by ${req.user?.role} (user #${approvedBy})`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId,
            ...
            notes: notes || null,
            hasAnalysisNote: !!loanAnalysisNote,
          },
        }).catch(err => console.error('Audit log error:', err));
        ...
        SecurityService.createAuditLog({
          ...
          organizationId,
          status: AuditStatus.FAILURE,
          description: `Loan reject-and-close failed: ${result.message}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: { loanId, reason: result.message },
        }).catch(err => console.error('Audit log error:', err));
  ...
  approveAndCloseLoan = async (
    ...
    try {
      ...
      if (result.success) {
        // ── Audit: loan approved and closed ──
        SecurityService.createAuditLog({
          ...
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan approved and closed by ${req.user?.role} (user #${approvedBy})`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId,
        ...
        SecurityService.createAuditLog({
          ...
          organizationId,
          status: AuditStatus.FAILURE,
          description: `Loan approve-and-close failed: ${result.message}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: { loanId, reason: result.message },
        }).catch(err => console.error('Audit log error:', err));
    ...
    try {
      ...
      if (result.success) {
        ...
        SecurityService.createAuditLog({
          ...
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan forwarded to reviewer by ${req.user?.role} (user #${reviewedBy}) — step ${workflowStep}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId,
  ...
  forwardToApprovalTeam = async (
    ...
    try {
      ...
      if (result.success) {
        // ── Audit: forwarded to approval team ──
        SecurityService.createAuditLog({
          action: AuditAction.LOAN_STATUS_CHANGED,
          resource: AuditResource.LOAN,
          resourceId: String(loanId),
          ...
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan forwarded to approval team by ${req.user?.role} (user #${reviewedBy})`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId,
  ...
  returnLoanForRework = async (
    ...
    try {
      ...
      if (result.success) {
        // ── Audit: loan returned for rework ──
        SecurityService.createAuditLog({
          action: AuditAction.LOAN_STATUS_CHANGED,
          resource: AuditResource.LOAN,
          resourceId: String(loanId),
          ...
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan returned for rework by ${req.user?.role} (user #${reviewedBy}) — type: ${returnType}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId,
  ...
  requestAdditionalDocuments = async (
    ...
    try {
      ...
      if (result.success) {
        // ── Audit: additional documents requested ──
        SecurityService.createAuditLog({
          ...
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Additional documents requested for loan #${loanId} by user #${req.user.id}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId,
            requestReason,
            requestedDocumentsCount: requestedDocuments?.length ?? 0,
          },
        }).catch(err => console.error('Audit log error:', err));
