...
// Add these new methods to your LoanApplicationController class

/**
 * ✅ NEW ENDPOINT 1: Forward loan to approval team
 * Managing Director forwards to Senior Manager and Board Director
 */
forwardToApprovalTeam = async (
  ...
  try {
    ...
    // Call service method (behaves like addLoanReview)
...
/**
 * ✅ NEW ENDPOINT 2: Return loan for rework
 * Managing Director, Board Director, or Senior Manager returns loan to loan officer
 * This does NOT affect loan status, only stores review message
 */
returnLoanForRework = async (
  ...
  try {
    ...
    if (req.user?.role === UserRole.CLIENT) {
      ...
      // For MANAGER role, need to determine if Board Director or Senior Manager
      // This would require additional logic or user metadata
      reviewerRole = WorkflowStep.BOARD_DIRECTOR; // Default for MANAGER
    }
    ...
    // Call service method - stores review without changing loan status
    const result = await this.loanApplicationService.addLoanReviewWithWorkflow(
      loanId,
      fullMessage,
      reviewedBy,
      organizationId,
      {
        ...
        decision: ReviewDecision.REQUEST_INFO, // Use REQUEST_INFO to indicate rework needed
        forwardToIds: null, // Not forwarding to specific users
        forwardToRoles: ['loan_officer'], // Indicating return to loan officer
