  ...
  createAnalysisReport = async (
    ...
    try {
      ...
      if (result.success) {
        ...
        // ── Audit: loan analysis report created ──
        SecurityService.createAuditLog({
          action: AuditAction.REPORT_GENERATED,
          resource: AuditResource.LOAN,
          resourceId: report?.reportId ?? String(reportData.loanId),
          userId,
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan analysis report created — type: ${reportData.reportType}, loan #${reportData.loanId}${
            report?.isFinalized ? ' (finalized — both signatures present)' : ''
          }${
            report?.isLoanOfficerSigned && !report?.isManagingDirectorSigned
              ? ' (pending managing director signature)'
              : ''
          }`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId:                   reportData.loanId,
            reportId:                 report?.reportId                   ?? null,
            reportType:               reportData.reportType,
            reportStatus:             report?.status                     ?? null,
            isFinalized:              report?.isFinalized                ?? false,
            isLoanOfficerSigned:      report?.isLoanOfficerSigned        ?? false,
            isManagingDirectorSigned: report?.isManagingDirectorSigned   ?? false,
            loanOfficerName:          reportData.loanOfficerName         ?? null,
            managingDirectorName:     reportData.managingDirectorName    ?? null,
            hasApprovalConditions:    !!approvalConditions,
            approvedAmount:           approvalConditions?.approvedAmount  ?? null,
            repaymentPeriod:          approvalConditions?.repaymentPeriod ?? null,
            interestRate:             approvalConditions?.interestRate    ?? null,
            hasRejectionReasons:      !!rejectionReasons,
            primaryRejectionReason:   rejectionReasons?.primaryReason    ?? null,
          },
        }).catch(err => console.error('Audit log error:', err));

        res.status(201).json(result);
      } else {
        // ── Audit: analysis report creation failed ──
        SecurityService.createAuditLog({
          action: AuditAction.REPORT_GENERATED,
          resource: AuditResource.LOAN,
          resourceId: String(reportData.loanId),
          userId,
          organizationId,
          status: AuditStatus.FAILURE,
          description: `Loan analysis report creation failed for loan #${reportData.loanId}: ${result.message}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            loanId:      reportData.loanId,
