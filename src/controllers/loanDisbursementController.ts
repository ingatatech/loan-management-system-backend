    ...
    try {
      ...
      if (result.success) {
        ...
        SecurityService.createAuditLog({
          ...
          resourceId: String(result.data?.loan?.id ?? disbursementData.loanId ?? ''),
          userId,
          organizationId,
          status: AuditStatus.SUCCESS,
          description: `Loan disbursed — account: ${disbursementData.borrowerAccountNumber}, net payable: ${result.data?.netAmountPayable}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            borrowerAccountNumber: disbursementData.borrowerAccountNumber,
            loanId: result.data?.loan?.loanId ?? disbursementData.loanId,
            approvedAmount: result.data?.loanDisbursement?.approvedAmount,
            netAmountPayable: result.data?.netAmountPayable,
            commissionRate: disbursementData.commissionRate,
            repaymentModality: disbursementData.repaymentModality,
            interestMethod: result.data?.calculatedValues?.interestMethod,
            totalInstallments: result.data?.calculatedValues?.totalInstallments,
          },
        }).catch(err => console.error('Audit log error:', err));

        res.status(201).json(result);
      } else {
        // ── Audit: disbursement failed ──
        SecurityService.createAuditLog({
          action: AuditAction.LOAN_DISBURSED,
          resource: AuditResource.LOAN,
          resourceId: String(disbursementData.loanId ?? ''),
          userId,
          organizationId,
          status: AuditStatus.FAILURE,
          description: `Loan disbursement failed: ${result.message}`,
          ipAddress: req.ip,
          userAgent: req.get('user-agent') ?? undefined,
          metadata: {
            borrowerAccountNumber: disbursementData.borrowerAccountNumber,
