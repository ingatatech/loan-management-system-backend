...
import { BorrowerProfile, BorrowerProfileData, Gender, MaritalStatus, RelationshipType } from "../entities/BorrowerProfile";
import { Loan, LoanData, InterestMethod, RepaymentFrequency, LoanStatus, LoanApprovalData } from "../entities/Loan";
...
async createCompleteLoanApplication( 
  ...
  console.log('=== CREATE LOAN APPLICATION (PENDING STATUS) ===');
  ...
  try {
    ...
    const borrowerIdCounter = await this.borrowerRepository.count({ where: { organizationId } });
    ...
    console.log('✓ Borrower created:', savedBorrower.id);

    // Generate unique loan ID
    const loanIdCounter = await this.loanRepository.count({ where: { organizationId } });
    ...
    console.log('✓ Loan created with PENDING status:', savedLoan.id);
    ...
    // Create collateral
    const collateralId = `COL-${organization.name.substring(0, 3).toUpperCase()}-${uuidv4().substring(0, 8).toUpperCase()}`;
    const collateral = this.collateralRepository.create({
      ...collateralData,
      collateralId,
      loanId: savedLoan.id,
      proofOfOwnershipUrl: uploadedUrls['proofOfOwnershipUrl'],
      ownerIdentificationUrl: uploadedUrls['ownerIdentificationUrl'],
      legalDocumentUrl: uploadedUrls['legalDocumentUrl'],
      physicalEvidenceUrl: uploadedUrls['physicalEvidenceUrl'],
      createdBy,
    });
    ...
    // ✅ FIX: Define savedGuarantor before if-block to maintain scope
    ...
    if (collateralData.guarantorName && collateralData.guarantorPhone) {
      ...
      savedGuarantor = await queryRunner.manager.save(guarantor);
      console.log('✓ Guarantor auto-created from collateral:', savedGuarantor.id);
    }

    await queryRunner.commitTransaction();
    console.log('✓ Transaction committed - Loan awaiting approval');

    const completeLoanApplication = await this.loanRepository.findOne({
    ...
    return {
      success: true,
      message: "Loan application created successfully and is pending approval",
      data: {
        loanApplication: completeLoanApplication,
    ...
    return { success: false, message: error.message || "Failed to create loan application" };
  } finally {
    await queryRunner.release();
  }
}
  ...
  async updateGuarantor(
    ...
    try {
      ...
      await queryRunner.manager.update(Loan, loanId, {
        ...
        agreedMaturityDate: approvalData.agreedMaturityDate,
...
async changeLoanStatus(
  ...
  try {
    ...
    return {
      success: true,
      message: `Loan status successfully changed from ${previousStatus} to ${newStatus}${
        emailSent ? ' and notification sent to borrower' : ''
      }`,
      data: {
        loan: updatedLoan,
        statusChange: {
