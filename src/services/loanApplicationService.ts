...
export interface BorrowerFiles {
  ...
  occupationSupportingDocuments?: Express.Multer.File[] | File[];
...
async createCompleteLoanApplication(
  ...
  try {
    ...
    const loan = queryRunner.manager.create(Loan, {
      ...
      loanRelevantDocuments: [], // Initialize as empty array
    ...
    // ✅ PROCESS LOAN RELEVANT DOCUMENTS
    if (borrowerFiles.loanRelevantDocuments?.length > 0 && createdBy) {
      ...
      try {
        ...
        console.error('Failed to parse loan relevant doc descriptions:', e);
      }

      const documentsWithDescriptions = borrowerFiles.loanRelevantDocuments.map((file, index) => ({
        description: descriptions[index] || `Loan Document ${index + 1}`,
        file
      }));
    ...
    const collateral = queryRunner.manager.create(LoanCollateral, {
      ...
      ownerInfo: collateralData.ownerInfo || null,
      marketValue: collateralData.marketValue || null,
      insuranceValue: collateralData.insuranceValue || null,
      forcedSaleValue: collateralData.forcedSaleValue || null,
    ...
    // ✅ CRITICAL FIX: RELOAD THE LOAN WITH ALL UPDATED DATA
    const finalLoan = await queryRunner.manager.findOne(Loan, {
      where: { id: savedLoan.id },
      relations: ['borrower', 'collaterals']
    });
    ...
    return {
      ...
      data: {
        loan: finalLoan || savedLoan, // Use reloaded loan with all documents
  ...
  async rejectAndCloseLoan(
    ...
    try {
      ...
      await queryRunner.manager.update(Loan, loanId, {
        ...
        rejectAndCloseAnalysisNote: loanAnalysisNote || null,
        updatedAt: now
      });
      ...
      const review = this.loanReviewRepository.create({
        ...
        loanAnalysisNote: loanAnalysisNote || null,
        reviewedAt: now
      });
  ...
  async approveAndCloseLoan(
    ...
    try {
      ...
      await queryRunner.manager.update(Loan, loanId, {
        ...
        approveAndCloseAnalysisNote: loanAnalysisNote || null,
        updatedAt: now
      });
  ...
  async getPendingLoanApplicationsWithWorkflow(
    ...
    try {
      ...
      // ✅ FIX: Batch-fetch ALL workflows for this page in ONE query
      //    instead of N individual queries inside Promise.all
      const loanIds = loans.map(loan => loan.id);

      let workflowsByLoanId = new Map<number, LoanWorkflow>();

      if (loanIds.length > 0) {
        const workflows = await this.workflowRepository
          .createQueryBuilder('workflow')
          .leftJoinAndSelect('workflow.currentAssignee', 'currentAssignee')
          .where('workflow.loanId IN (:...loanIds)', { loanIds })
          .getMany();

        for (const wf of workflows) {
          workflowsByLoanId.set(wf.loanId, wf);
        }
      }

      // ✅ FIX: Pure synchronous map — no async, no per-loan DB calls
      const loansWithWorkflow = loans.map((loan) => {
        const workflow = workflowsByLoanId.get(loan.id) ?? null;

        // ✅ PRESERVED: Original analysis report summary
        // ✅ FIX: use slice() before sort() to avoid mutating the original array
        const analysisReportSummary = loan.analysisReports && loan.analysisReports.length > 0 ? {
          ...
          latestReport: loan.analysisReports
            .slice()
        ...
        // ✅ PRESERVED: Build complete guarantor information array
        ...
        // ✅ PRESERVED: Guarantor summary statistics
