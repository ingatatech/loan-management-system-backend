...
import { Address, BorrowerProfile, BorrowerProfileData, Gender, MaritalStatus, RelationshipType } from "../entities/BorrowerProfile";
import { Loan, LoanData, InterestMethod, RepaymentFrequency, LoanStatus, LoanApprovalData, IncomeFrequency, BorrowerType, InstitutionProfile, SpouseInfo, IncomeSourceInfo, InstitutionType, ShareholderBoardMemberInfo } from "../entities/Loan";
...
export interface InstitutionFiles {
  ...
  // ✅ NEW: Shareholder/Board Member file fields
...
export interface EnhancedLoanApplicationRequest {
  ...
  // Individual borrower data (existing)
  borrowerData?: BorrowerProfileData;

  // Institution data (new)
  institutionData?: {
  ...
  // Loan data (existing but enhanced)
  loanData: Omit<LoanData, 'borrowerId'> & {
    businessOfficer: string;
  };

  // Collateral data (existing)
...
export interface EnhancedCollateralFiles extends CollateralFiles {
  ...
  institutionLegalDocument?: Express.Multer.File[]; // RDB, RGB, RCA

  // Spouse and witness documents
  marriageCertificate?: Express.Multer.File[];
  spouseCrbReport?: Express.Multer.File[];
  witnessCrbReport?: Express.Multer.File[];
...
// ===== COMPLETE createCompleteLoanApplication METHOD =====
// 100% ORIGINAL FUNCTIONALITY PRESERVED + NEW MULTIPLE GUARANTORS FEATURE
...
async createCompleteLoanApplication(
    ...
    loanData: Omit<LoanData, 'borrowerId'> & {
      ...
      shareholderBoardMembers?: ShareholderBoardMemberInfo[];
    },
    collateralData: CollateralData & {
    ...
    console.log('=== ENHANCED CREATE LOAN APPLICATION ===');
    console.log('Borrower Type:', loanData.borrowerType || BorrowerType.INDIVIDUAL);
  console.log('📁 Files received:', {

    collateralFiles: collateralFiles ? Object.keys(collateralFiles) : [],
    guarantorFiles: guarantorFiles ? Object.keys(guarantorFiles) : [],
    borrowerFiles: borrowerFiles ? Object.keys(borrowerFiles) : [],
    institutionFiles: institutionFiles ? Object.keys(institutionFiles) : [],
  });
    try {
      await queryRunner.connect();
      await queryRunner.startTransaction();

      // ===== STEP 1: Verify Organization (100% ORIGINAL) =====
      const organization = await this.organizationRepository.findOne({
        where: { id: organizationId }
      });
      if (!organization) throw new Error("Organization not found");

      // ===== STEP 2: Create Borrower Profile (100% ORIGINAL) =====
      const borrowerIdCounter = await this.borrowerRepository.count({
        where: { organizationId }
      });
      const borrowerId = `BOR-${organization.name.substring(0, 3).toUpperCase()}-${String(borrowerIdCounter + 1).padStart(6, '0')}`;

      const borrowerProfileData = {
        ...borrowerData,
        borrowerId,
        organizationId,
        createdBy,
      ...
      const borrowerProfile = this.borrowerRepository.create(borrowerProfileData);
      const savedBorrower = await queryRunner.manager.save(borrowerProfile);
      console.log('✓ Borrower created:', savedBorrower.id, 'Type:', loanData.borrowerType);

      // ===== STEP 3: Upload Borrower-Specific Documents (100% ORIGINAL) =====
      const borrowerDocUrls: Record<string, string> = {};

      // Individual borrower documents
      if (loanData.borrowerType === BorrowerType.INDIVIDUAL && borrowerFiles) {
        if (borrowerFiles.marriageCertificate && borrowerFiles.marriageCertificate.length > 0) {
          const result = await UploadToCloud(borrowerFiles.marriageCertificate[0]);
          borrowerDocUrls.marriageCertificate = result.secure_url;
          console.log('✓ Marriage certificate uploaded');
        }

        if (loanData.maritalStatus === MaritalStatus.MARRIED &&
          borrowerFiles.spouseCrbReport && borrowerFiles.spouseCrbReport.length > 0) {
          const result = await UploadToCloud(borrowerFiles.spouseCrbReport[0]);
          borrowerDocUrls.spouseCrbReport = result.secure_url;
          console.log('✓ Spouse CRB report uploaded');
        }

        if (loanData.maritalStatus === MaritalStatus.MARRIED &&
          borrowerFiles.spouseIdentification && borrowerFiles.spouseIdentification.length > 0) {
          const result = await UploadToCloud(borrowerFiles.spouseIdentification[0]);
          borrowerDocUrls.spouseIdentification = result.secure_url;
          console.log('✓ Spouse identification uploaded');
        }

        if (borrowerFiles.witnessCrbReport && borrowerFiles.witnessCrbReport.length > 0) {
          const result = await UploadToCloud(borrowerFiles.witnessCrbReport[0]);
          borrowerDocUrls.witnessCrbReport = result.secure_url;
          console.log('✓ Witness CRB report uploaded');
        }

        if (borrowerFiles.witnessIdentification && borrowerFiles.witnessIdentification.length > 0) {
          const result = await UploadToCloud(borrowerFiles.witnessIdentification[0]);
          borrowerDocUrls.witnessIdentification = result.secure_url;
          console.log('✓ Witness identification uploaded');
        }
      }

      // ===== STEP 4: Upload Institution Documents (100% ORIGINAL) =====
      const institutionDocUrls: Record<string, string> = {};

      if (loanData.borrowerType === BorrowerType.INSTITUTION && institutionFiles) {
        const institutionType = loanData.institutionProfile?.institutionType;

        if (institutionType === InstitutionType.COMPANY && institutionFiles.institutionLegalDocument) {
          if (institutionFiles.institutionLegalDocument.length > 0) {
            const result = await UploadToCloud(institutionFiles.institutionLegalDocument[0]);
            institutionDocUrls.legalDocument = result.secure_url;
            console.log('✓ RDB document uploaded (Company)');
          }
        } else if (institutionType === InstitutionType.COOPERATIVE && institutionFiles.cooperativeLegalDocument) {
          if (institutionFiles.cooperativeLegalDocument.length > 0) {
            const result = await UploadToCloud(institutionFiles.cooperativeLegalDocument[0]);
            institutionDocUrls.legalDocument = result.secure_url;
            console.log('✓ RGB document uploaded (Cooperative)');
          }
        } else if (institutionType === InstitutionType.OTHER && institutionFiles.otherInstitutionLegalDocument) {
          if (institutionFiles.otherInstitutionLegalDocument.length > 0) {
            const result = await UploadToCloud(institutionFiles.otherInstitutionLegalDocument[0]);
            institutionDocUrls.legalDocument = result.secure_url;
            console.log('✓ RCA document uploaded (Other Institution)');
          }
        }

        if (institutionFiles.institutionLicense && institutionFiles.institutionLicense.length > 0) {
          const result = await UploadToCloud(institutionFiles.institutionLicense[0]);
          institutionDocUrls.license = result.secure_url;
          console.log('✓ Institution license uploaded');
        }

        if (institutionFiles.institutionTradingLicense && institutionFiles.institutionTradingLicense.length > 0) {
          const result = await UploadToCloud(institutionFiles.institutionTradingLicense[0]);
          institutionDocUrls.tradingLicense = result.secure_url;
          console.log('✓ Institution trading license uploaded');
        }

        if (institutionFiles.institutionRegistration && institutionFiles.institutionRegistration.length > 0) {
          const result = await UploadToCloud(institutionFiles.institutionRegistration[0]);
          institutionDocUrls.registration = result.secure_url;
          console.log('✓ Institution registration uploaded');
        }
      }

 
   const loanIdCounter = await this.loanRepository.count({
      where: { organizationId }
    });
    const loanId = `LOAN-${organization.name.substring(0, 3).toUpperCase()}-${String(loanIdCounter + 1).padStart(6, '0')}`;

      // Prepare conditional data
      let spouseInfoWithDocs = null;
      let institutionProfileWithDocs = null;

      if (loanData.borrowerType === BorrowerType.INDIVIDUAL && loanData.spouseInfo) {
        spouseInfoWithDocs = {
          ...loanData.spouseInfo,
          crbReportUrl: borrowerDocUrls.spouseCrbReport || loanData.spouseInfo.crbReportUrl,
        };
      }

      if (loanData.borrowerType === BorrowerType.INSTITUTION && loanData.institutionProfile) {
        institutionProfileWithDocs = {
          ...loanData.institutionProfile,
          legalDocumentUrl: institutionDocUrls.legalDocument || loanData.institutionProfile.legalDocumentUrl,
          licenseUrl: institutionDocUrls.license,
          tradingLicenseUrl: institutionDocUrls.tradingLicense,
          registrationUrl: institutionDocUrls.registration,
        };
      }
    ...
    // Prepare shareholder/board member data with uploaded files
    let enrichedShareholderBoardMembers = null;
    
    if (loanData.borrowerType === BorrowerType.INSTITUTION && 
        loanData.shareholderBoardMembers && 
        loanData.shareholderBoardMembers.length > 0) {
      
      console.log('📋 Enriching shareholder/board member data with file URLs...');
      
      enrichedShareholderBoardMembers = loanData.shareholderBoardMembers.map((member, index) => ({
        ...member,
        documents: {
          // Map uploaded files to member documents
          identificationUrl: shareholderBoardMemberDocs.shareholderIdentification?.[index] || 
                           shareholderBoardMemberDocs.boardMemberIdentification?.[index],
          proofOfSharesUrl: shareholderBoardMemberDocs.proofOfShares?.[index],
          boardResolutionUrl: shareholderBoardMemberDocs.boardResolution?.[index],
          crbReportUrl: shareholderBoardMemberDocs.shareholderCrbReport?.[index] || 
                       shareholderBoardMemberDocs.boardMemberCrbReport?.[index],
        }
      }));
      
      console.log(`✅ Enriched ${enrichedShareholderBoardMembers.length} shareholder/board members with documents`);
    }
     ...
     const loan = this.loanRepository.create({
      ...
      // Conditional fields
      borrowerType: loanData.borrowerType || BorrowerType.INDIVIDUAL,

      // Individual-specific fields
      ...(loanData.borrowerType === BorrowerType.INDIVIDUAL && {
        ...
        marriageCertificateUrl: borrowerDocUrls.marriageCertificate || null,
        spouseInfo: spouseInfoWithDocs,
        witnessCrbReportUrl: borrowerDocUrls.witnessCrbReport || null,
        incomeSources: loanData.incomeSources || null,
      }),

      // Institution-specific fields with enriched shareholder data
      ...(loanData.borrowerType === BorrowerType.INSTITUTION && {
        institutionProfile: institutionProfileWithDocs,
        shareholderBoardMembers: enrichedShareholderBoardMembers, // ✅ FIX: Use enriched data
      }),
    ...
    if (enrichedShareholderBoardMembers) {
      console.log(`   With ${enrichedShareholderBoardMembers.length} shareholder/board members and their documents`);
    }
      // ===== STEP 6: Upload Collateral Documents (100% ORIGINAL) =====
      const uploadedCollateralUrls: Record<string, string[]> = {};

      if (collateralFiles) {
        for (const [fieldName, files] of Object.entries(collateralFiles)) {
          if (files && files.length > 0) {
            uploadedCollateralUrls[fieldName] = [];

            for (const file of files) {
              const result = await UploadToCloud(file);
              uploadedCollateralUrls[fieldName].push(result.secure_url);
              console.log(`✓ File uploaded for ${fieldName}:`, result.secure_url);
            }
          }
        }
      }

      // ===== STEP 7: Create Collateral (100% ORIGINAL) =====
      const collateralIdValue = `COL-${organization.name.substring(0, 3).toUpperCase()}-${uuidv4().substring(0, 8).toUpperCase()}`;

      const collateral = this.collateralRepository.create({
        ...collateralData,
        collateralId: collateralIdValue,
        loanId: savedLoan.id,
        proofOfOwnershipUrls: uploadedCollateralUrls['proofOfOwnership'] ?
          JSON.stringify(uploadedCollateralUrls['proofOfOwnership'].map(url => ({
            url,
            uploadedAt: new Date().toISOString(),
            type: 'proofOfOwnership'
          }))) : null,
        ownerIdentificationUrls: uploadedCollateralUrls['ownerIdentification'] ?
          JSON.stringify(uploadedCollateralUrls['ownerIdentification'].map(url => ({
            url,
            uploadedAt: new Date().toISOString(),
            type: 'ownerIdentification'
          }))) : null,
        legalDocumentUrls: uploadedCollateralUrls['legalDocument'] ?
          JSON.stringify(uploadedCollateralUrls['legalDocument'].map(url => ({
            url,
            uploadedAt: new Date().toISOString(),
            type: 'legalDocument'
          }))) : null,
        physicalEvidenceUrls: uploadedCollateralUrls['physicalEvidence'] ?
          JSON.stringify(uploadedCollateralUrls['physicalEvidence'].map(url => ({
            url,
            uploadedAt: new Date().toISOString(),
            type: 'physicalEvidence'
          }))) : null,
        valuationReportUrls: uploadedCollateralUrls['valuationReport'] ?
          JSON.stringify(uploadedCollateralUrls['valuationReport'].map(url => ({
            url,
            uploadedAt: new Date().toISOString(),
            type: 'valuationReport'
          }))) : null,
        additionalDocumentsUrls: uploadedCollateralUrls['additionalCollateralDocs'] ?
          JSON.stringify(uploadedCollateralUrls['additionalCollateralDocs'].map(url => ({
            url,
            uploadedAt: new Date().toISOString(),
            type: 'additional',
      ...
      const savedCollateral = await queryRunner.manager.save(collateral);
      console.log('✓ Collateral saved:', savedCollateral.id);

      // ========================================
      // ✅ NEW: STEP 8 - CREATE MULTIPLE GUARANTORS
      // ========================================
      let savedGuarantors: Guarantor[] = [];
      ...
      // ✅ BACKWARD COMPATIBILITY: Single guarantor from collateralData (100% ORIGINAL)
      if (collateralData.guarantorName && collateralData.guarantorPhone) {
        guarantorsData.push({
          name: collateralData.guarantorName,
          phone: collateralData.guarantorPhone,
          address: collateralData.guarantorAddress,
          guaranteedAmount: collateralData.collateralValue,
        });
        console.log('✓ Single guarantor from collateral data (backward compatibility)');
      }

      // ✅ NEW: Multiple guarantors from guarantorsData array
      if (collateralData.guarantorsData && Array.isArray(collateralData.guarantorsData)) {
        guarantorsData = [...guarantorsData, ...collateralData.guarantorsData];
        console.log(`✓ Additional guarantors from guarantorsData: ${collateralData.guarantorsData.length}`);
      }

      console.log(`📋 Processing ${guarantorsData.length} guarantor(s)...`);

      // ✅ NEW: Loop through and create each guarantor
      for (let i = 0; i < guarantorsData.length; i++) {
        const guarantorData = guarantorsData[i];
        
        const guarantorIdDocs: string[] = [];
        
        // Upload guarantor identification documents
        if (guarantorFiles?.guarantorIdentification) {
          const filesArray = Array.isArray(guarantorFiles.guarantorIdentification) 
            ? guarantorFiles.guarantorIdentification 
            : [guarantorFiles.guarantorIdentification];
          
          for (const file of filesArray) {
            const result = await UploadToCloud(file);
            guarantorIdDocs.push(result.secure_url);
            console.log(`✓ Guarantor ${i + 1} ID uploaded:`, result.secure_url);
          }
        }

        // Upload guarantor CRB report
        let guarantorCrbUrl = null;
        if (guarantorFiles?.guarantorCrbReport && guarantorFiles.guarantorCrbReport.length > 0) {
          // For backward compatibility, use first file if only one guarantor
          const fileIndex = guarantorsData.length === 1 ? 0 : i;
          if (guarantorFiles.guarantorCrbReport[fileIndex]) {
            const result = await UploadToCloud(guarantorFiles.guarantorCrbReport[fileIndex]);
            guarantorCrbUrl = result.secure_url;
            console.log(`✓ Guarantor ${i + 1} CRB report uploaded`);
          }
        }
        ...
        const guarantor = this.guarantorRepository.create({
          ...
          guaranteedAmount: guarantorData.guaranteedAmount || collateralData.collateralValue,
          collateralType: collateralData.collateralType,
          collateralDescription: collateralData.description,
          
          // ✅ NEW: Extended fields
          nationalId: guarantorData.nationalId || null,
          guarantorType: guarantorData.guarantorType || 'individual',
          
          identificationDocumentUrls: guarantorIdDocs.length > 0 ?
            JSON.stringify(guarantorIdDocs.map(url => ({
              url,
              uploadedAt: new Date().toISOString(),
              type: 'identification'
            }))) : null,
        ...
        savedGuarantors.push(saved);
        console.log(`✓ Guarantor ${i + 1} created:`, saved.id);
      }

      console.log(`✅ Total guarantors created: ${savedGuarantors.length}`);


     console.log('🔍 STEP 8B: PROCESS SHAREHOLDER/BOARD MEMBER FILES');
    
    const shareholderBoardMemberDocs: Record<string, string[]> = {};
    
    if (loanData.borrowerType === BorrowerType.INSTITUTION && institutionFiles) {
      console.log('🏢 Processing institution shareholder/board member files...');
      
      // Define shareholder/board member file fields
      const shareholderFileFields = [
      ...
      for (const fieldName of shareholderFileFields) {
        const files = (institutionFiles as any)[fieldName];
        
        if (files && Array.isArray(files) && files.length > 0) {
          shareholderBoardMemberDocs[fieldName] = [];
          
          console.log(`   Processing ${files.length} file(s) for ${fieldName}`);
          
          for (const file of files) {
            try {
              const uploadResult = await UploadToCloud(file);
              shareholderBoardMemberDocs[fieldName].push(uploadResult.secure_url);
      ...
      console.log(`✅ Shareholder/Board member files processed:`, 
        Object.keys(shareholderBoardMemberDocs).length, 'file types');
    }
      ...
      if (
        ...
        console.log('💑 PHASE 1: Auto-creating spouse as guarantor...');

        try {
          const spouseGuarantor = this.guarantorRepository.create({
            loanId: savedLoan.id,
            collateralId: savedCollateral.id,
            borrowerId: savedBorrower.id,
            organizationId: organizationId,

            // Basic guarantor fields
            name: `${loanData.spouseInfo.firstName} ${loanData.spouseInfo.lastName}`,
            phone: loanData.spouseInfo.phone,
            address: borrowerData.address ? JSON.stringify(borrowerData.address) : '',
            guaranteedAmount: loanData.disbursedAmount,
            collateralType: 'personal_guarantee',
            collateralDescription: 'Spouse automatic guarantee',

            // Extended guarantor fields for spouse
            nationalId: loanData.spouseInfo.nationalId,
            surname: loanData.spouseInfo.lastName,
            forename1: loanData.spouseInfo.firstName,
            guarantorType: 'individual',

            createdBy,
          });

          await queryRunner.manager.save(spouseGuarantor);
          spouseGuarantorCreated = true;
          console.log('✅ PHASE 1: Spouse auto-saved as guarantor');
        } catch (spouseError) {
          console.error('❌ PHASE 1: Failed to auto-save spouse as guarantor:', spouseError);
        }
      }

      // ========================================
      // ✅ ORIGINAL: PHASE 1: SHAREHOLDER/BOARD MEMBER HANDLING (100% PRESERVED)
      // ========================================
      ...
      if (
        ...
        console.log('🏢 PHASE 1: Saving shareholder/board members...');

        try {
          (savedLoan as any).shareholderBoardMembers = loanData.shareholderBoardMembers;
          await queryRunner.manager.save(savedLoan);

          shareholdersCount = loanData.shareholderBoardMembers.length;
          console.log(`✅ PHASE 1: Saved ${shareholdersCount} shareholder/board members`);
        } catch (shareholderError) {
          console.error('❌ PHASE 1: Failed to save shareholders/board members:', shareholderError);
        }
      }

      // ===== COMMIT TRANSACTION (100% ORIGINAL) =====
       await queryRunner.commitTransaction();
    console.log('✓ Transaction committed');

    // ===== PREPARE SUCCESS RESPONSE =====
    const successMessage =
      loanData.borrowerType === BorrowerType.INDIVIDUAL && spouseGuarantorCreated
        ? "Loan application created successfully (spouse automatically added as guarantor)"
        : loanData.borrowerType === BorrowerType.INSTITUTION && shareholdersCount > 0
          ? `Loan application created successfully (${shareholdersCount} shareholder/board members with documents added)`
          : savedGuarantors.length > 1
            ? `Loan application created successfully with ${savedGuarantors.length} guarantors`
            : "Loan application created successfully and is pending approval";
    ...
    return {
      ...
      data: {
        ...
        guarantor: savedGuarantors.length === 1 ? savedGuarantors[0] : null,
        guarantors: savedGuarantors,
        guarantorsCount: savedGuarantors.length,
        spouseGuarantorCreated: spouseGuarantorCreated,
        shareholdersCount: shareholdersCount,
        shareholderDocumentsUploaded: Object.keys(shareholderBoardMemberDocs).length > 0, // ✅ NEW
        uploadedFiles: {
          collateral: uploadedCollateralUrls,
          borrower: borrowerDocUrls,
          institution: institutionDocUrls,
          shareholderBoardMember: shareholderBoardMemberDocs, // ✅ NEW
        },
    ...
    try {
      ...
      if (updateData.firstName || updateData.lastName || updateData.email) {
        ...
        const borrowerFields = [
          ...
          'dateOfBirth', 'maritalStatus', 'primaryPhone', 'alternativePhone',
  ...
  /**
   * Add loan review with workflow context
   */
  async addLoanReviewWithWorkflow(
    ...
    try {
      ...
      const review = this.loanReviewRepository.create({
        ...
        forwardedToId: workflowData.forwardedToId,
        reviewedAt: new Date()
      });

      const savedReview = await queryRunner.manager.save(review);
      console.log('✓ Review saved with workflow context:', savedReview.id);
      ...
      // 7. Send email notifications (existing logic)
      try {
        const usersToNotify = await this.userRepository.find({
          where: [
            { organizationId, role: UserRole.CLIENT, isActive: true },
            { organizationId, role: UserRole.MANAGER, isActive: true }
          ]
        });
        ...
        const emailPromises = usersToNotify
          ...
          .map(user =>
            sendLoanReviewedEmail(
              user.email!,
              `${user.firstName || ''} ${user.lastName || user.username}`.trim(),
              user.role === UserRole.CLIENT ? 'client' : 'manager',
              borrowerName,
              loan.loanId,
              loan.disbursedAmount,
              reviewerName,
      ...
      return {
        success: true,
        message: "Review added successfully with workflow integration",
        data: {
          review: completeReview,
          reviewCount,
          emailsSent: true,
      ...
      return {
        success: false,
        message: error.message || "Failed to add loan review with workflow",
      };
    } finally {
      await queryRunner.release();
    }
