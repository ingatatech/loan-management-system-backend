...
async createCompleteLoanApplication(
    ...
    try {
        ...
        // ✅ CRITICAL FIX: Extract relationship type from borrowerData (ensure it's not undefined)
        const relationshipType = borrowerData.relationshipWithNDFSP || RelationshipType.NEW_BORROWER;
        console.log(`🔍 Relationship Type from request: ${relationshipType}`);
        
        // ✅ CRITICAL FIX: Extract source borrower ID from loanData if provided
        const sourceBorrowerIdFromRequest = loanData.sourceBorrowerId;
        console.log(`🔍 Source Borrower ID from request: ${sourceBorrowerIdFromRequest}`);
        ...
        // ✅ ENHANCED: Check for existing borrower based on multiple criteria
        console.log(`🎯 Checking for existing borrower with multiple criteria...`);
        console.log(`   Relationship Type: ${relationshipType}`);
        console.log(`   Source Borrower ID: ${sourceBorrowerIdFromRequest}`);
        console.log(`   Existing Borrower ID: ${existingBorrowerId}`);
        console.log(`   National ID: ${borrowerData.nationalId}`);
        
        // ===== PRIORITY 1: Use sourceBorrowerId if provided (from auto-population) =====
        let foundBorrowerId = sourceBorrowerIdFromRequest || existingBorrowerId;
        ...
        // ✅ CRITICAL FIX: Check for REPEAT_BORROWER client account
        console.log(`🔍 Checking relationship type for client account: ${relationshipType}`);
        if (relationshipType === RelationshipType.REPEAT_BORROWER) {
            console.log('🔍 STEP 2.1: CHECKING FOR EXISTING CLIENT BORROWER ACCOUNT FOR REPEAT BORROWER');
            ...
            console.log(`   Searching client account for: ${identifier} (${borrowerType})`);
            ...
            if (existingClientAccount) {
                console.log(`✅ Found existing client account: ${existingClientAccount.accountNumber}`);
                console.log(`   Account holder: ${existingClientAccount.borrowerNames || existingClientAccount.institutionName}`);
                console.log(`   Client account borrower ID: ${existingClientAccount.borrowerId}`);
                console.log(`   Current borrower ID: ${savedBorrower?.id}`);
                
                // ✅ VERIFY: Ensure the client account belongs to the same borrower
                if (savedBorrower && existingClientAccount.borrowerId !== savedBorrower.id) {
                    console.log(`⚠️ WARNING: Client account borrower ID (${existingClientAccount.borrowerId}) doesn't match current borrower ID (${savedBorrower.id})`);
                    console.log(`   Updating client account to point to current borrower...`);
                    existingClientAccount.borrowerId = savedBorrower.id;
                    await queryRunner.manager.save(ClientBorrowerAccount, existingClientAccount);
                }
            } else {
                console.log('❌ ERROR: Repeat borrower relationship type detected but no client account found');
                throw new Error(`Cannot process repeat borrower without existing client account. National ID: ${borrowerData.nationalId}`);
            }
        } else {
            console.log(`ℹ️ Relationship type is ${relationshipType}, skipping client account check`);
        }

        const borrowerType = loanData.borrowerType || BorrowerType.INDIVIDUAL;

        // ===== STEP 2.2: OCCUPATION SUPPORTING DOCUMENTS =====
...
// ===== ✅ CRITICAL FIX: UPDATE CLIENT BORROWER ACCOUNT FOR REPEAT BORROWERS =====
if (hasClientAccount && existingClientAccount) {
    console.log('🔄 STEP 3.1: UPDATING EXISTING CLIENT BORROWER ACCOUNT');
    console.log(`   Updating account: ${existingClientAccount.accountNumber}`);
    
    // ✅ CRITICAL: Link the new loan to this client account via foreign key
    ...
    console.log(`✅ Loan ${savedLoan.loanId} linked to client account ${existingClientAccount.accountNumber}`);
    
    // ✅ UTILIZE 1-TO-MANY: Fetch all loans for this account
    const allAccountLoans = await queryRunner.manager.find(Loan, {
        where: { 
    ...
    console.log(`📊 Total loans for this account: ${allAccountLoans.length}`);
    console.log(`📋 Loan IDs: ${allAccountLoans.map(l => l.loanId).join(', ')}`);
    ...
    if (borrowerType === BorrowerType.INDIVIDUAL) {
        ...
        existingClientAccount.borrowerNames = updatedNames;
        existingClientAccount.nationalId = borrowerData.nationalId;
        
        // ✅ Update profile information with ALL changes + loan history
        existingClientAccount.profileInformation = {
            ...(existingClientAccount.profileInformation || {}),
            ...
            // ✅ ADD: Track loan history
            totalLoans: allAccountLoans.length,
            allLoanIds: allAccountLoans.map(l => l.loanId)
        };
        
        console.log(`✅ Updated individual client account fields`);
        console.log(`   Total loans tracked: ${allAccountLoans.length}`);
    } else {
        // Update institution borrower account
        existingClientAccount.institutionName = loanData.institutionProfile?.institutionName;
        existingClientAccount.tinNumber = loanData.institutionProfile?.tinNumber;
        existingClientAccount.businessNumber = loanData.institutionProfile?.licenseNumber;
        
        existingClientAccount.institutionInformation = {
            ...(existingClientAccount.institutionInformation || {}),
            ...
            lastLoanId: savedLoan.loanId,
            lastLoanDate: new Date().toISOString(),
            // ✅ ADD: Track loan history for institutions
            totalLoans: allAccountLoans.length,
            allLoanIds: allAccountLoans.map(l => l.loanId)
        };
        
        if (loanData.institutionProfile?.contactPerson) {
            existingClientAccount.profileRepresentative = {
    ...
    // ✅ SAVE UPDATED CLIENT ACCOUNT
    await queryRunner.manager.save(ClientBorrowerAccount, existingClientAccount);
    console.log('✅ CLIENT BORROWER ACCOUNT SUCCESSFULLY UPDATED');
    console.log(`   Account Number: ${existingClientAccount.accountNumber}`);
    console.log(`   New Loan Added: ${savedLoan.loanId}`);
    console.log(`   All Loans: ${allAccountLoans.map(l => l.loanId).join(', ')}`);
