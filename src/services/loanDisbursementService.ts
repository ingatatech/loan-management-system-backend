...
export class UpdatedLoanDisbursementService {
  ...
  async createThreeStepDisbursement(
    ...
    try {
      ...
      // ✅ NEW: Upload mortgage registration certificate if provided
      ...
      const mortgageRegistration = queryRunner.manager.create(MortgageRegistration, {
        ...
        mortgageRegistrationCertificateUrl: mortgageCertificateUrl, // ✅ NEW: Add certificate URL
        organizationId,
        createdBy
      });
      ...
      // ✅ ENHANCED: Calculate due dates starting from today
      ...
      // ✅ ENHANCED: GENERATE REPAYMENT SCHEDULE WITH CORRECT DUE DATES
  ...
  private async generateRepaymentScheduleByModality(
    ...
    // ✅ ENHANCED: Always calculate from today
    const today = new Date();
  ...
  // ========================================
  // ✅ ENHANCED: SINGLE PAYMENT GENERATOR
  // ========================================
  ...
  private generateInterestOnlySchedule(loan: Loan, firstPaymentDate: Date): RepaymentSchedule[] {
    ...
    // Generate interest-only payments (all except last)
    for (let i = 1; i < totalTerms; i++) {
      ...
      installment.dueDate = dueDate;
      installment.duePrincipal = 0; // No principal payment
      installment.dueInterest = Math.round(periodicInterest * 100) / 100;
      installment.dueTotal = installment.dueInterest;
      installment.outstandingPrincipal = principal; // Principal unchanged
      installment.status = ScheduleStatus.PENDING;
      installment.paidPrincipal = 0;
      installment.paidInterest = 0;
      installment.paidTotal = 0;
  ...
  private generateCustomizedSchedule(
    ...
    console.log('🎯 Generating customized schedule');
    ...
    if (customTotal < minTotal) {
      throw new Error(
        ...
        `Please add ${(minTotal - customTotal).toFixed(2)} RWF to cover principal and interest.`
      );
    }
    ...
    customSchedule.forEach((item, index) => {
      ...
      // Allocate interest (simple heuristic: 10-15% of payment or proportional)
      let interestPortion: number;
      if (isLast) {
        interestPortion = minInterest - totalInterestAllocated;
      } else {
        interestPortion = Math.min(item.amount * 0.15, minInterest / customSchedule.length);
      }

      const principalPortion = Math.min(item.amount - interestPortion, remainingPrincipal);
    ...
    if (Math.abs(remainingPrincipal) > 0.01) {
      console.warn(`⚠️  Remaining principal: ${remainingPrincipal.toFixed(2)} (should be ~0)`);
    }

    console.log(`✓ Generated ${schedule.length} customized payments`);

    return schedule;
  }

  // ========================================
  // ✅ ENHANCED: STANDARD REPAYMENT SCHEDULE
  // ========================================
