...
class DemoRequestService {
  ...
  async createDemoRequest(data: CreateDemoRequestData): Promise<ServiceResponse<DemoRequest>> {
    try {
      ...
      sendDemoRequestNotification(savedRequest).catch(err => {
  ...
  async updateDemoRequestStatus(
    ...
    try {
      const request = await this.demoRequestRepository.findOne({
        where: { id }
      });
  ...
  async getDemoRequestStats(): Promise<ServiceResponse<any>> {
    try {
      ...
      const pending = await this.demoRequestRepository.count({
        where: { status: DemoRequestStatus.PENDING }
      });
      const contacted = await this.demoRequestRepository.count({
        where: { status: DemoRequestStatus.CONTACTED }
      });
      const completed = await this.demoRequestRepository.count({
        where: { status: DemoRequestStatus.COMPLETED }
      });
      const spam = await this.demoRequestRepository.count({
        where: { status: DemoRequestStatus.SPAM }
      });
